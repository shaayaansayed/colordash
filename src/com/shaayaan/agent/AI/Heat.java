...
public class Heat extends Agent {
	...
	public Heat(Point pos, int marker, State state) {
	...
	public Move getNextMove(State state) {
		...
		for (int i = 0; i < numFruits; i++) {
			...
			deadFruits[i] = myFruits[i] > (double)(totalFruits[i]/2) || oppFruits[i] > (double)(totalFruits[i]/2) ? 1 : 0; 
		}
		...
		if (max == 0) {
			Point nearestCoord = findNearestFruit(state, myX, myY); 
	...
	public Point findNearestFruit(State state, int myX, int myY) {
		int boardWidth = state.getWidth(); 
		int boardHeight = state.getHeight();
		int bound = 1; 
		Point nearestCoord = new Point(0, 0); 
		while (true) {
